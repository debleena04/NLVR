...
class Modules:
    def __init__(self, image_feat_grid, word_vecs, encoder_states, num_choices, map_dim, batch_size):
        ...
        att_summary = [att_shape[0]]+[3,3]
        input_att_summary = tf.placeholder(tf.float32,att_summary)
    ...
    def _slice_flag(self,flag,batch_idx):
        print("batch idx shape",batch_idx.shape)
        flag_mapped =  tf.gather(flag,batch_idx)
        return tf.reshape(flag_mapped, [])
    ...
    def BreakModule(self, time_idx, batch_idx, flag, map_dim=500, scope='BreakModule',
        reuse=True):
        #global flag
        print("Flag in break before setting",flag)
        #self.flag = True
        flag =True
    ...
    def FindModule(self, time_idx, batch_idx, flag, map_dim=32, scope='FindModule',
        ...
        #global flag
        # In TF Fold, batch_idx and time_idx are both [N_batch, 1] tensors
        print("Flag value in find",flag, "type of flag in find", type(flag))
            ...
            for i in range(3):
               with tf.variable_scope(self.module_variable_scope):
                  with tf.variable_scope(scope, reuse=reuse):
                      ...
                      print("image_feat_mapped in find true",image_feat_mapped.shape)
        ...
        #att_grid = tf.cond(tf.equal(flag,1), if_true, if_false)
        #flag_current = tf.cast(tf.gather(flag, batch_idx),dtype = tf.bool)
        #N = tf.shape(time_idx)[0]
        #flag = tf.reshape(flag, to_T([N,1]))
        #flag_current = self._slice_flag(flag,batch_idx)
        #flag_current = tf.gather(flag, batch_idx)
        #flag_current = tf.gather(flag,[0])
        #flag_current = tf.reshape(flag, [])
        #print("flag type in flag:",(flag_current))
        #flag_current = tf.cast(flag_current,dtype = tf.bool)
        #print("Flag current shape",flag_current.shape)
        #att_grid = tf.cond(tf.convert_to_tensor(flag_current==1), if_true, if_false)
        att_grid = tf.cond(tf.reduce_all(tf.equal(flag,tf.constant(1))), if_true, if_false)
        #att_grid = tf.cond(flag_current, if_true, if_false)
        #att_grid = tf.cond(tf.equal(tf.reshape(flag,[]), tf.constant(1)), if_true, if_false)
        return att_grid
        ...
        def if_false():
            ...
            with tf.variable_scope(self.module_variable_scope):
               with tf.variable_scope(scope, reuse=reuse):
                   ...
                   att_grid = tf.tile(att_grid,[1,3,1,1,1])
            return att_grid
        #att_grid = tf.cond(tf.reshape(tf.equal(flag,1),[]), if_true, if_false)
        #att_grid = tf.cond(tf.convert_to_tensor(flag==1), if_true, if_false)
        #flag_current = tf.cast(tf.gather(flag, batch_idx),dtype = tf.bool)
        #N = tf.shape(time_idx)[0]
        #flag = tf.reshape(flag, to_T([N,1]))
        #flag_current = self._slice_flag(flag,batch_idx)
        #flag_current = tf.gather(flag, batch_idx)
        print("Flag type in transform:", flag)
        #flag_current = tf.gather(flag,[0])
        #flag_current = tf.reshape(flag, [])
        #flag_current = tf.cast(flag_current,dtype = tf.bool)
        #print("Flag current shape",flag_current.shape)
        #att_grid = tf.cond(tf.convert_to_tensor(flag_current==1), if_true, if_false)
        #att_grid = tf.cond(flag_current==1, if_true, if_false)
        #att_grid = tf.cond(tf.reshape(flag_current,[]), if_true, if_false)
        #att_grid = tf.cond(tf.cast(flag_current,tf.bool), if_true, if_false)
        att_grid = tf.cond(tf.reduce_all(tf.equal(flag, tf.constant(1))), if_true, if_false)
        #att_grid = tf.cond(flag,tf.reshape(tf.constant(1),[]), if_true, if_false)
        return att_grid
    

    def DescribeModule(self, input_0, time_idx, batch_idx, flag, scope='DescribeModule', reuse=True):
        # In TF Fold, batch_idx and time_idx are both [N_batch, 1] tensors
        #image_feat_grid = self._slice_image_feat_grid(batch_idx)
        print("Flag value in describe", flag)
    ...
    def AndModule(self, input_0, input_1, time_idx, batch_idx, flag,
    ...
    def OrModule(self, input_0, input_1, time_idx, batch_idx, flag,
    ...
    def NotModule(self, input_0, time_idx, batch_idx, flag,
    ...
    def CountModule(self, input_0, time_idx, batch_idx, flag, map_dim=32, 
        ...
        print("Flag value in count",flag)
        #global flag
        with tf.variable_scope(self.module_variable_scope):
                    with tf.variable_scope(scope, reuse=reuse):
                        H, W = self.att_shape[1:3]
                        with tf.variable_scope('fc_scores'):
        ...
        def if_true():
            ...
            for i in range(3):
                with tf.variable_scope(self.module_variable_scope):
                    with tf.variable_scope(scope, reuse=reuse):
                        #H, W = image_feat_grid_arr[0].get_shape().as_list()[2:4]
                        H, W = self.att_shape[1:3]
            ...
            scores = tf.convert_to_tensor(scores_arr)
            scores = tf.transpose(scores,[1,0,2])
            print("shape of return score in count:",scores.shape)
            return scores  
        ...
        #score = tf.cond(tf.convert_to_tensor(flag==1), if_true, if_false)
        #flag_current = tf.cast(tf.gather(flag, batch_idx),dtype = tf.bool)
        #N = tf.shape(time_idx)[0]
        #flag = tf.reshape(flag, to_T([N,1]))
        #flag_current = self._slice_flag(flag,batch_idx)
        ...
        #flag_current = tf.cast(flag_current,dtype = tf.bool)
        #print("Flag current shape",flag_current.shape)
        #score = tf.cond(tf.convert_to_tensor(flag_current==1), if_true, if_false)
        #score = tf.cond(tf.reshape(flag_current,[]), if_true, if_false)
        #score = tf.cond(flag_current==1, if_true, if_false)
        #score = tf.cond(tf.equal(tf.reshape(flag,[]), tf.constant(1)), if_true, if_false)
    ...
    def AttReduceModule(self, input_0, time_idx, batch_idx, flag, map_dim=32,
        ...
        print("Flag value in attreduced", flag)
        with tf.variable_scope(self.module_variable_scope):
            with tf.variable_scope(scope, reuse=reuse):
    ...
    def CompareModule(self, input_0, time_idx, batch_idx, flag, map_dim=32, scope='CompareModule',
        ...
        print("Flag value in Compare",flag)
        image_feat_grid = self._slice_image_feat_grid(batch_idx)
        image_feat_grid = tf.pad(image_feat_grid, tf.convert_to_tensor([[0,0],[0,0],[1,1],[0,0]]),'CONSTANT')
        text_param = self._slice_word_vecs(time_idx, batch_idx)
        ...
        def if_true():
            ...
            scores = tf.convert_to_tensor(scores_arr)
            scores = tf.transpose(scores,[1,0,2])
            print("shape of return score in compare:",scores.shape)
            return scores 
        ...
        def if_false():
            ...
            with tf.variable_scope(self.module_variable_scope):
                with tf.variable_scope(scope, reuse=reuse):
                    ...
                    #scores.set_shape((1,map_dim))
                    scores = tf.tile(scores,[1,3,1])
            print("shape of return score in compare from false:",scores.shape)
            return scores
        #score = tf.cond(tf.reshape(tf.equal(flag,1),[]), if_true, if_false)
        #score = tf.cond(tf.convert_to_tensor(flag==1), if_true, if_false)
        #flag_current = tf.cast(tf.gather(flag, batch_idx),dtype = tf.bool)
    ...
    def CompareReduceModule(self, input_0, input_1, time_idx, batch_idx, flag, map_dim=32, scope='CompareReduceModule',
        ...
        D_im = input_0.get_shape().as_list()[-1]
        print("Flag value in comparereduce", flag)
        #D_txt = text_param.get_shape().as_list()[-1]
        with tf.variable_scope(self.module_variable_scope):
           with tf.variable_scope(scope, reuse=reuse):
               with tf.variable_scope('fc_eltwise'):
    ...
    def CompareAttModule(self, input_0, input_1, time_idx, batch_idx, flag, map_dim=32, scope='CompareAttModule',
        ...
        print("Flag value in compareatt",flag)
        #global flag
        #D_im = input_0.get_shape().as_list()[-1]
        ...
        with tf.variable_scope(self.module_variable_scope):
           with tf.variable_scope(scope, reuse=reuse):
               with tf.variable_scope('fc_eltwise'):
                   W_e = tf.get_variable('weights', [3, 1],
                    initializer=tf.contrib.layers.xavier_initializer())
                   b_e = tf.get_variable('biases', 1,
                    initializer=tf.constant_initializer(0.))
        ...
        def if_true():
            ...
            scores = []
            print("shape", image_feat_grid_arr0)
            for i in range(3):
                with tf.variable_scope(self.module_variable_scope):
                    with tf.variable_scope(scope, reuse=reuse):
                        ...
                        #scores = tf.tile(scores,[1,3])
                        scores_arr.append(scores)'''
                        min_d = tf.reduce_min(image_feat_grid_arr_1[i])
                        max_d = tf.reduce_max(image_feat_grid_arr_1[i])
                        att_normalised1 = (image_feat_grid_arr_1[i] - min_d) / (max_d - min_d)
                        min_d = tf.reduce_min(image_feat_grid_arr_2[i])
                        max_d = tf.reduce_max(image_feat_grid_arr_2[i])
                        att_normalised2 = (image_feat_grid_arr_2[i] - min_d) / (max_d - min_d)
                        att_and = tf.minimum(att_normalised1, att_normalised2)
                        att_min = tf.reduce_min(att_and, axis=[1, 2])
                        att_avg = tf.reduce_mean(att_and, axis=[1, 2])
                        att_max = tf.reduce_max(att_and, axis=[1, 2])
                        print("at_max shape",att_max.shape)
                        att_reduced = tf.concat([att_min, att_avg, att_max], axis=1)
        ...
        def if_false():
            ...
            min_d = tf.reduce_min(input_0)
            max_d = tf.reduce_max(input_0)
            att_normalised1 = (input_0 - min_d) / (max_d - min_d)
            min_d = tf.reduce_min(input_1)
            max_d = tf.reduce_max(input_1)
            att_normalised2 = (input_1 - min_d) / (max_d - min_d)
            att_and = tf.minimum(att_normalised1, att_normalised2)
            att_min = tf.reduce_min(att_and, axis=[1, 2, 3])
            att_avg = tf.reduce_mean(att_and, axis=[1, 2, 3])
            att_max = tf.reduce_max(att_and, axis=[1, 2,3])
            att_reduced = tf.concat([att_min, att_avg, att_max], axis=1)
            print("att_reduced",att_reduced.shape)
            with tf.variable_scope(self.module_variable_scope):
                with tf.variable_scope(scope, reuse=reuse):
    ...
    def CombineModule(self, input_0, time_idx, batch_idx, flag, map_dim=32, scope='CombineModule',
        ...
        #   2. L2-normalization
        #   3. Linear classification
        print("Flag value in combine", flag)
        N = tf.shape(time_idx)[0]
        text_param = self._slice_word_vecs(time_idx, batch_idx)
    ...
    def ExistAttModule(self, input_0, time_idx, batch_idx, flag, map_dim=32, scope='ExistAttModule',
        ...
        #   2. L2-normalization
        #   3. Linear classification
        print("Flag value in existatt", flag)
        print("input in existatt", input_0.shape)
        image_feat_grid_arr0, image_feat_grid_arr1, image_feat_grid_arr2 = tf.split(input_0, num_or_size_splits=3, axis=1)
        with tf.variable_scope(self.module_variable_scope):
            with tf.variable_scope(scope, reuse=reuse):
                N = tf.shape(time_idx)[0]
                ...
                print("text_paramsoftmax shape",text_param_softmax)
                with tf.variable_scope('token_prediction'):
                    w_y = tf.get_variable('weights', [3,1],
                       initializer=tf.contrib.layers.xavier_initializer())
                    b_y = tf.get_variable('biases', 1,
                    initializer=tf.constant_initializer(0.))
                    print("image feat in existatt",image_feat_grid_arr0.shape)
    ...
    def ExistModule(self, input_0, time_idx, batch_idx, flag, scope='ExistModule', reuse=True):
        # In TF Fold, batch_idx and time_idx are both [N_batch, 1] tensors
