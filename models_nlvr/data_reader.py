...
import models_nlvr.text_processing as text_processing
...
class BatchLoader:
    def __init__(self, data, data_params):
        ...
        self.load_gt_txtatt = ('gt_txtatt' in self.data[0]) and (self.data[0]['gt_txtatt'] is not None)
        ...
        if not self.load_answer:
            print('data does not contain answers')
        else:
        ...
        if self.load_gt_layout:
            ...
            print('data does not contain ground-truth layout, and "overriding_layout" key is not set')
    ...
    def load_one_batch(self, sample_ids):
        ...
        image_data_batch = np.zeros((actual_batch_size, self.feat_H, self.feat_W, self.feat_D), np.float32)
        ...
        for n in range(len(sample_ids)):
            ...
            image_data_batch[n:n+1] = iminfo['image_data']
            ...
            if self.load_answer:
                ...
                label = iminfo['label']
                answer_idx = self.answer_dict.word2idx(label)
        ...
        batch = dict(input_seq_batch=input_seq_batch,
                     ...
                     image_data_batch=image_data_batch,
...
class DataReader:
    def __init__(self, data_file, shuffle=True, one_pass=False, prefetch_num=8, **kwargs):
        print('Loading data from file...', end=''); sys.stdout.flush()
...
def _run_prefetch(prefetch_queue, batch_loader, data, shuffle, one_pass, data_params):
